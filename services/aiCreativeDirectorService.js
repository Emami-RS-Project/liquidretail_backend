...
const DIRECTOR_SIGNALS_VERSION = '3.7.0';   // BUMPED 2026-09-08: product_signal.content_sufficiency + marketing_line (provenance-aware) + pdpSpecFacts/pdpMaterialFacts/pdpFaqAnswers filling product_signal.specs + quotes[].stage carried through. CONTENT_ATOM_READ gates the new keys and the prompt sentences (flag-off Meta prompt stays byte-identical). Also primes contentInventory before quote dual-read so proof_options match the renderer. Live path re-assembles every round; the bump is the SHADOW directConcepts cache-hit gate (one paid re-derive per cached product).
// 3.6: brand_signal.personas from Brand.demographics (DIRECTOR_BRAND_PERSONAS). Voice/objection framing only — never quote authors. Flag-off omits the key and the PERSONAS rule so inputSummary + round prompt stay byte-identical.
...
// authored — generated copy, not a first-party slogan. Mirror of
// contentCompiler.marketingLineProvenance (kept local so Director does not
// import the compiler).
...
async function assembleSignals({ brandId, productId, campaignKind, seededUniverse } = {}) {
  ...
  // has no LLM). The sync dual-read (loadPrintableQuoteAtomsForProduct)
  // only sees the in-process Map this primes. Mint order is Director
  // THEN layout assembleInput, so without this prime the Director ranks
  // Mixed while the renderer hydrates atoms.
  ...
  // CONTENT_ATOM_READ: sufficiency + marketing_line are NEW keys — omit
  // entirely with the flag off so JSON.stringify of the signal stays
  // byte-identical to 3.6.0. Flag-on always sets them when the product
  // actually holds the data (never a dummy empty object).
...
module.exports = {
  ...
  // exposed for testing
  catalogSpecsForDirector,
  marketingLineForDirector,
  contentSufficiencyForDirector,
  marketingLineProvenanceForDirector,
  buildPromptRound,
