...
- **Catalog review intake (`services/catalogReviewIntakeService.js`, 2026-09-07).** All 4 ingest paths now start a free on-site review scrape (JSON-LD → vendor API → optional headless, `catalogProductReviewRefreshService.refreshOne`) BEFORE the existing YOLO/`enqueueBrandProductDetects` enqueue, fire-and-forget. The lazy per-SKU path (`productMatchService.maybeFetchProductReviewsCached`) now tries the same scraper before its Gemini fallback too — Gemini only fires on `no-canonical-url`/`no-data`/`scraper-error`, never `not-found`. **Its per-run product cap is its OWN `CATALOG_REVIEW_INTAKE_MAX_STEPS` (file default 10000) — deliberately NOT the manual Tier-4 workflow's `MAX_STEPS_PER_RUN=100`.** That 100 bounds a human's single synchronous SSE confirm-click; this hook is unattended background work and should cover a brand's whole catalog on first ingest. Do not re-couple the two constants. Write-up: `session.d/2026-09-07_catalog-review-intake-scraper-first.md`.
- **`refreshOne` also captures description/brand/sku/gtin/mpn/image/specs/slug/slogan (2026-09-07), same JSON-LD `Product` node, zero extra cost.** All gap-fill-only except `slug` (always refreshed — mechanical URL derivation, not curated). Two real hazards found+fixed by adversarial review: (1) a page with multiple `Product`-typed nodes (carousels/variants) could attribute the WRONG product's identity fields — fixed via `pickPrimaryProductNode` (URL-match or unambiguous-single-node only, never a blind merge); (2) an unvalidated JSON-LD `image` could be a `data:`/relative/dangerous URL that gets mirrored and marked generation-ready — fixed via `resolveAbsoluteImageUrl`. `brand.name` off JSON-LD is often a generic placeholder ("Apparel" on live Pelagic pages) — only accepted when it resembles the row's own `Brand.name`. `slug` = URL path segment (e.g. `mako-shorts`), NOT a marketing slogan — `slogan` is the separate, new, scraped-only (never LLM-generated) field for that. Do not run a second full-catalog bulk sweep to test a scraper change — `refreshOne` already captures everything in one fetch; a same-day repeat sweep on Pelagic tripped an external rate limit. Write-up: `session.d/2026-09-07_catalog-product-info-scrape-live-findings.md`.
