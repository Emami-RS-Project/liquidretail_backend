...
**GEN-1 — Brand `styleScript` is executed, not sandboxed → authenticated-tenant RCE. FIXED (guard landed).** `services/brandScriptRunner.child.js:130-143` calls `vm.compileFunction` with no `parsingContext`; the parameter list controls the function's arguments, not its scope chain, so the body resolves `globalThis`/`process`/`process.mainModule.require` against the live V8 context. Reachable by any tenant via `POST /api/brand/:id/preview-script` → `brandScriptExecutor.js` `runChild`. **SECURITY.**

*Correction to this entry's original line refs and fix:* the route line is `routes/brand.js:1136` (not `:1147`) and the executor branch is `:795` (not `:772`). More importantly, **the originally prescribed fix — deleting the `bodyScript` branch — was insufficient and would have left the hole open as a two-request exploit.** Three doors reached the VM, not one:

1. `body.script` → forces `engine='canvas'` at `:1136`
2. `body.engine:'canvas'` → the `bodyEngine` escape hatch at `:1132`, which short-circuits **before** `resolveTitlingEngine` is consulted
3. a `styleScript` / `styleScriptVertical` / `styleScriptLandscape` persisted earlier via `PATCH /api/brand/:id` (allow-listed at `:264-267` with **no validation**), then previewed with `{engine:'canvas'}` and no `body.script` at all — landing at `:1169-1170`

*Fix as landed:* a single `if (engine !== 'remotion') return 400` guard immediately after the engine resolution at `:1136-1138`. It closes all three doors, stays closed if `resolveTitlingEngine` is ever un-hardwired, and modifies no existing line. Nothing live regressed: the only UI caller, `StyleOverridesCard.tsx`, is commented out of `Brand/index.tsx`. Pinned by `scripts/verifyPreviewScriptGuard.js` (8 checks, revert-proven — removing the guard fails 3).
