...
- `POST /title-still` — the FAST refinement loop: body {format, spec?, frames? (≤4 sec marks), scale?, meta? (text fields only)}; synchronous, ~1-3s warm via the stills fast lane (enqueueStill — never waits behind a production render). Powers `GET /title-playground` (public/titlePlayground.html: sliders, AI box, save).
