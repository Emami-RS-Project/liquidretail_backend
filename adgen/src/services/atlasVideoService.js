...
async function generateForAd({
  ...
  // '720p'. Distinct from retryOverride (an object that pins prompt/refs/
  // model/aspect AND resolution for the automatic QC-retry). On the
  // NORMAL (non-retryOverride) path this wins over env/caps defaults;
  // when retryOverride is set, retryOverride.resolution still wins and
  // this parameter is ignored. Every existing caller omits it.
