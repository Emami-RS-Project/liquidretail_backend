...
try {
  ...
  // SUPERSEDED 2026-09-07 (owner decision): the CTA button is removed from
  // EVERY static surface unconditionally — resolveDrawCta returns false
  // ahead of SURFACE_POLICY and both PMax flags, superseding the 2026-08-24
  // PMAX_STATIC_CTA_ALL_INTENTS decision these checks used to pin. Inverted
  // again rather than deleted, same discipline as before: a later unreviewed
  // revert to "CTA everywhere" must fail a test immediately.
  check('C8 PMax draws no in-image CTA for social_proof_led (2026-09-07: CTA removed everywhere)',
    drawCta('pmax_square_1_1', 'social_proof_led') === false);
  check('C9 PMax draws no in-image CTA for objection_resolved (2026-09-07: CTA removed everywhere)',
    drawCta('pmax_square_1_1', 'objection_resolved') === false);
  check('C10 Meta also draws no CTA for either intent (2026-09-07: CTA removed everywhere)',
    drawCta('meta_feed_4_5', 'social_proof_led') === false &&
    drawCta('meta_feed_4_5', 'objection_resolved') === false);
  restoreEnv();
  ...
  // TRIPWIRE RE-FIRED AND ACKNOWLEDGED — 2026-09-07. Residual 2 was CLOSED
  // 2026-08-24 by granting PMax the in-image CTA on every intent; it is now
  // MOOT rather than closed-in-that-sense, because the 2026-09-07 owner
  // decision removed the CTA from every static surface, PMax included. There
  // is no longer an "allowlist" to include social_proof_led in — the button
  // does not exist on any intent, on any surface.
  //
  // WHAT THIS DOES NOT DECIDE — read before widening anything. Neither the
  // 2026-08-24 grant nor its 2026-09-07 removal touched the Director
  // reservation gate (aiCreativeDirectorService.js, correction 1), which
  // remains an explicit, unstarted owner call — the D-group markers below
  // still pin the gate's current rating-only reasoning. Do not read either
  // CTA decision as consent for that separate question.
  check('D3 residual 2 now MOOT — PMax draws no CTA for any intent, social_proof_led included',
    drawCta('pmax_square_1_1', 'social_proof_led') === false,
    'The 2026-09-07 decision removed the CTA from every static surface. If this is red, ' +
    'the CTA-removal fix has been (partially) reverted — that is a change to a decided ' +
    'behaviour, not a harness bug. The Director reservation-gate question (correction 1 in ' +
    'src/services/aiCreativeDirectorService.js) is still open and was NOT decided by that change.');
